...
begin
  if (TabForm <> nil) and TabForm.HandleAllocated then
    PostMessage(TabForm.Handle, CEFBROWSER_INITIALIZED, 0, 0);
end;
...
procedure TTabForm.AddTabBtnClick(Sender: TObject);
var
  TempSheet: TTabSheet;
...
procedure TTabForm.RemoveTabBtnClick(Sender: TObject);
var
  TempChromium: TChromium;
begin
...
procedure TTabForm.CloseAllBrowsers;
var
  i, j, k: integer;
...
procedure TTabForm.ForwardBtnClick(Sender: TObject);
var
  TempChromium: TChromium;
begin
  if SearchChromium(PageControl1.TabIndex, TempChromium) then
...
procedure TTabForm.GoBtnClick(Sender: TObject);
var
  TempChromium: TChromium;
begin
  if SearchChromium(PageControl1.TabIndex, TempChromium) then
...
procedure TTabForm.ReloadBtnClick(Sender: TObject);
var
  TempChromium: TChromium;
begin
  if SearchChromium(PageControl1.TabIndex, TempChromium) then
...
procedure TTabForm.BackBtnClick(Sender: TObject);
var
  TempChromium: TChromium;
begin
  if SearchChromium(PageControl1.TabIndex, TempChromium) then
...
procedure TTabForm.StopBtnClick(Sender: TObject);
var
  TempChromium: TChromium;
begin
  if SearchChromium(PageControl1.TabIndex, TempChromium) then
...
procedure TTabForm.tmr1Timer(Sender: TObject);
begin
  Destroy;
end;

procedure TTabForm.BrowserCreatedMsg(var aMessage: TMessage);
var
  TempWindowParent: TCEFWindowParent;
  TempChromium: TChromium;
begin
...
procedure TTabForm.BrowserDestroyWindowParentMsg(var aMessage: TMessage);
var
  TempWindowParent: TCEFWindowParent;
begin
  if SearchWindowParent(aMessage.lParam, TempWindowParent) then
...
procedure TTabForm.BrowserDestroyTabMsg(var aMessage: TMessage);
begin
...
procedure TTabForm.BrowserCheckTaggedTabsMsg(var aMessage: TMessage);
begin
  if (aMessage.lParam >= 0) and (aMessage.lParam < PageControl1.PageCount) then
  begin
...
function TTabForm.AllTabSheetsAreTagged: Boolean;
var
  i: integer;
begin
  Result := True;
...
procedure TTabForm.Chromium_OnAfterCreated(Sender: TObject;
  const browser: ICefBrowser);
var
...
procedure TTabForm.Chromium_OnAddressChange(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame; const url: ustring);
var
  TempPageIndex: integer;
begin
  if not(FClosing) and (PageControl1.TabIndex >= 0) and
    GetPageIndex(Sender, TempPageIndex) and
    (PageControl1.TabIndex = TempPageIndex) then
    URLCbx.Text := url;
end;

function TTabForm.GetPageIndex(const aSender: TObject;
  var aPageIndex: integer): Boolean;
begin
  Result := False;
  aPageIndex := -1;
...
procedure TTabForm.Chromium_OnTitleChange(Sender: TObject;
  const browser: ICefBrowser; const title: ustring);
var
  TempPageIndex: integer;
begin
...
procedure TTabForm.Chromium_OnClose(Sender: TObject; const browser: ICefBrowser;
  var aAction: TCefCloseBrowserAction);
var
  TempPageIndex: integer;
begin
...
procedure TTabForm.Chromium_OnBeforeClose(Sender: TObject;
  const browser: ICefBrowser);
var
  TempPageIndex: integer;
begin
...
procedure TTabForm.Chromium_OnBeforePopup(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
  const targetUrl, targetFrameName: ustring;
  targetDisposition: TCefWindowOpenDisposition; userGesture: Boolean;
  const popupFeatures: TCefPopupFeatures; var windowInfo: TCefWindowInfo;
...
function TTabForm.SearchChromium(aPageIndex: integer;
  var aChromium: TChromium): Boolean;
var
  i, j: integer;
  TempComponent: TComponent;
...
function TTabForm.SearchWindowParent(aPageIndex: integer;
  var aWindowParent: TCEFWindowParent): Boolean;
var
  i, j: integer;
...
procedure TTabForm.NotifyMoveOrResizeStarted;
var
  i, j: integer;
  TempChromium: TChromium;
begin
...
procedure TTabForm.WMMove(var aMessage: TWMMove);
begin
  inherited;
...
procedure TTabForm.WMMoving(var aMessage: TMessage);
begin
  inherited;
...
procedure TTabForm.WMEnterMenuLoop(var aMessage: TMessage);
begin
  inherited;
...
procedure TTabForm.WMExitMenuLoop(var aMessage: TMessage);
begin
  inherited;
...
procedure TTabForm.PageControl1Change(Sender: TObject);
var
  TempChromium: TChromium;
begin
...
procedure TTabForm.CEFInitializedMsg(var aMessage: TMessage);
begin
...
procedure TTabForm.AddLink(const Link: string);
begin
  // AddTabBtn.Click;
  URLCbx.Text := Link;
  GoBtn.Click;
